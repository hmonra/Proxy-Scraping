...
from scrap6 import driver
from scrap6 import summary_item
from scrap6 import hora_inicial
...
def comprar():
    ...
    if driver2.current_url == 'https://www.pccomponentes.com/cart/':
        telegram_bot_sendtext("BOT 6 - ❌ Intento de compra realizado, sin stock ❌:  " + nombre_producto)
        print("Compra no realizada, el stock ha volado... :(")
    else:
        if driver2.current_url == 'https://www.pccomponentes.com/cart/summary':
            telegram_bot_sendtext("BOT 6 - ❌ Intento de compra realizado ❌:  " + nombre_producto)
            print("Compra no realizada, algo ha fallado... :(")
        else:
            telegram_bot_sendtext("BOT 6 - ✅ Compra realizada ✅:  " + nombre_producto)
...
while True:
    ...
    try:
        ...
        if "0" not in cuadro_cesta:
            ...
            print("INSTANCIA 6 - 🔴 Ninguna gráfica en stock...      Hora:", current_time)
            print("Script corriendo desde: ", hora_inicial)
            driver2.minimize_window()
            subprocess.call("bucle5.py", shell=True)
    except TimeoutException or WebDriverException:
        print("")
        print("Url Time out..")
