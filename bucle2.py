...
from scrap2 import driver
from scrap2 import summary_item
from scrap2 import hora_inicial
...
def comprar():
    ...
    if driver2.current_url == 'https://www.pccomponentes.com/cart/':
        telegram_bot_sendtext("BOT 2 - ❌ Intento de compra realizado, sin stock ❌:  " + nombre_producto)
        print("Compra no realizada, el stock ha volado... :(")
    else:
        if driver2.current_url == 'https://www.pccomponentes.com/cart/summary':
            telegram_bot_sendtext("BOT 2 - ❌ Intento de compra realizado ❌:  " + nombre_producto)
            print("Compra no realizada, algo ha fallado... :(")
        else:
            telegram_bot_sendtext("BOT 2 - ✅ Compra realizada ✅:  " + nombre_producto)
...
while True:
    ...
    try:
        ...
        if "0" not in cuadro_cesta:
            ...
            print("INSTANCIA 2 - 🔴 Ninguna gráfica en stock...      Hora:", current_time)
            print("Script corriendo desde: ", hora_inicial)
            driver2.minimize_window()
            subprocess.call("bucle1.py", shell=True)
    except TimeoutException or WebDriverException:
        print("")
        print("Url Time out..")
