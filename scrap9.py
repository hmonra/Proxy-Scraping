...
# PROXY
# options.add_argument("--proxy-server=195.114.202.189:58542")
...
driver2 = webdriver.Chrome(executable_path=r'RUTA_CHROMEDRIVER', # AÑADIR AQUÍ CHROMEDRIVER
                          options=options)
...
def comprar():
    ...
    if driver2.current_url == 'https://www.pccomponentes.com/cart/':
        telegram_bot_sendtext("BOT 9 - ❌ Intento de compra realizado, sin stock ❌:  " + nombre_producto)
        print("Compra no realizada, el stock ha volado... :(")
    else:
        if driver2.current_url == 'https://www.pccomponentes.com/cart/summary':
            telegram_bot_sendtext("BOT 9 - ❌ Intento de compra realizado ❌:  " + nombre_producto)
            print("Compra no realizada, algo ha fallado... :(")
        else:
            telegram_bot_sendtext("BOT 9 - ✅ Compra realizada ✅:  " + nombre_producto)
    ...
    while True:
        ...
        try:
            ...
            if "0" not in cuadro_cesta:
                ...
                print("INSTANCIA 9 - 🔴 Ninguna gráfica en stock...      Hora:", current_time)
                print("Script corriendo desde: ", hora_inicial)
                driver2.minimize_window()
                os.system('python scrap8.py')
        except TimeoutException or WebDriverException:
            print("")
            print("Url Time out..")
...
print("Login para precargar página summary...")
driver2.switch_to.window(driver2.window_handles[0])
driver2.get('https://www.pccomponentes.com/')
enter_proxy_auth()
driver2.minimize_window()
